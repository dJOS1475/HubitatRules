 ...
 *  1.2.3-ts 2026-06-29    (test build)     TEST: Restored proactive sendTimeSync() (removed in 1.2.2) on configure(), ZDO Node Descriptor request, and device announcement.  Single-variable A/B test to confirm whether the lost proactive Time cluster push is what reintroduced the motion-reporting stalls in 1.2.2.  All other 1.2.2 behavior unchanged.
 *
 */

static String version()   { "1.2.3-ts" }
static String timeStamp() { "2026/06/29 12:00" }
    ...
    switch (descMap.clusterId) {
        ...
        case "0002": // Node Descriptor Request (Node_Desc_req)
            ...
            */
            ...
            runIn(1, "sendTimeSync")   // RESTORED: proactive Time cluster push (keep-alive for motion subsystem)
...
void sendTimeSync() {
    ...
    // RESTORED in v1.2.3-timesync-test (removed in Dan's v1.2.2) – suspected keep-alive for the motion subsystem.
...
void configure() {
    ...
    runIn(DEFAULT_POLLING_INTERVAL, "deviceHealthCheck", [overwrite: true, misfire: "ignore"])
    runIn(1, "sendTimeSync", [overwrite: true])   // RESTORED: proactive Time cluster push (keep-alive for motion subsystem)
